...
var gemeindeListReferenz = [];

function referenzAdresse(searchResults, reisezeiten) {
    ...
    d3.select('#messageReferenz').text('Nächste Gemeinde: ' + nearestGemeinde.name + ' (Distanz zur Eingabe: ' + nearestGemeinde.distance.toFixed(3) + ' km )')
        .style('float', 'left')
        .style('clear', 'both')
    ...
    gemeindeListReferenz.push(nearestGemeinde);
    redrawEverythingReferenz();

    return nearestGemeinde;

}

function redrawEverythingReferenz() {
    var divs = d3.select('#resultsReferenz')
        .selectAll('div')
        .data(gemeindeListReferenz);
